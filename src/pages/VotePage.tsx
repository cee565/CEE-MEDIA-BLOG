...
import { CheckCircle2, BarChart3, Share2, Link, Check, Heart, Clock, ArrowLeft, ChevronRight } from 'lucide-react';
...
const PollCard = React.memo(({ poll }: { poll: Poll }) => {
  ...
  return (
    <motion.div 
      ...
      className="bg-white/40 backdrop-blur-2xl rounded-[1rem] card-shadow p-3 md:p-6 space-y-4 border border-white/60 overflow-hidden group/card relative"
    >
        ...
        <div className="flex flex-col lg:flex-row lg:items-start justify-between gap-4">
          <div className="space-y-3 flex-1">
            ...
            <div className="flex flex-wrap items-center gap-2.5 pt-0.5">
              {timeLeft && !isEnded && (
                <div className="flex items-center space-x-1.5 bg-white/50 backdrop-blur-md text-purple-600 px-3 py-1.5 rounded-lg border border-purple-100 shadow-sm group/timer">
                  <Clock size={14} className="animate-pulse group-hover:scale-110 transition-transform" />
                  <span className="text-[9px] font-black uppercase tracking-[0.15em]">{timeLeft}</span>
                </div>
              )}
              {isEnded && !poll.image && (
      ...
      <div className="grid grid-cols-1 gap-4">
        {poll.options.map((option, index) => {
          ...
          return (
            <button
              ...
              className={`w-full relative overflow-hidden rounded-[1rem] p-3 md:p-5 text-left transition-all duration-500 group/opt border-2 ${
                showResults 
                  ? 'cursor-default border-transparent' 
                  : 'hover:shadow-2xl border-white/40 hover:border-purple-400 active:scale-[0.98] bg-white/30 backdrop-blur-md'
              } ${
                isSelected 
              ...
              {showResults && (
                <motion.div
                  ...
                  className={`absolute top-0 left-0 h-full ${
                    isSelected 
                      ? 'bg-gradient-to-r from-purple-600/20 to-purple-400/5' 
                      : 'bg-gradient-to-r from-slate-200/50 to-slate-100/20'
                  }`}
                />
              )}
                ...
                <div className="flex items-center space-x-3.5">
                  {!showResults && (
                    <div className={`w-5 h-5 rounded-full border-2 flex items-center justify-center transition-all duration-500 ${
                      ...
                      {isPending && <Check size={12} className="text-white" />}
                    </div>
                  )}
                  {optionImage && (
                    <div className="w-14 h-14 rounded-xl overflow-hidden border-2 border-white/60 flex-shrink-0 shadow-xl group-hover/opt:scale-110 transition-transform duration-500">
                      <img src={optionImage} alt={optionText} className="w-full h-full object-cover" referrerPolicy="no-referrer" loading="lazy" />
                    </div>
                  )}
                  <div className="flex flex-col space-y-0.5">
                    <span className={`text-base md:text-lg font-black tracking-tight ${isSelected || isPending ? 'text-purple-800' : 'text-slate-800'}`}>
                      {optionText}
                    </span>
                    {showResults && (
                ...
                {showResults && (
                  <div className="flex items-center space-x-3.5">
                    <div className="flex flex-col items-end">
                      <span className={`text-xl font-black ${isSelected ? 'text-purple-600' : 'text-slate-600'}`}>
                        {percentage}%
                      </span>
                    </div>
                    {isSelected && (
                      <motion.div
                        ...
                        className="bg-purple-600 p-1.5 rounded-full shadow-xl shadow-purple-200"
                      >
        ...
        {!showResults && tempSelected !== null && (
          <motion.button
            ...
            className="w-full py-3.5 bg-gradient-to-r from-purple-600 via-indigo-600 to-blue-600 text-white rounded-[1rem] font-black text-base shadow-[0_20px_50px_rgba(124,58,237,0.3)] hover:shadow-[0_25px_60px_rgba(124,58,237,0.4)] hover:scale-[1.02] active:scale-[0.98] transition-all disabled:opacity-50 flex items-center justify-center space-x-2.5 mt-4 group/btn"
          >
            {isSubmitting ? (
                    ...
                    {pollGroups.map((group, index) => (
                      ...
                      >
                        ...
                        <div className="relative z-10 flex items-center justify-between pt-4 mt-auto">
                          <div className="flex items-center space-x-1.5 bg-white/80 px-3 py-1.5 rounded-lg shadow-sm border border-white/60">
                          ...
                          <div className="flex items-center space-x-1 text-purple-600 font-black text-[8px] uppercase tracking-[0.2em] group-hover:translate-x-2 transition-transform duration-500">
                            <span>Explore</span>
